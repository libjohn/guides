...
2. **Find all literal words**  <br>*Simple searches are just like traditional find & replace*
    + `avian` -- 2 matches <br><img src="http://www.clipular.com/c/6697213840326656.png?k=Sr4JJIBaQIQrFV_e22vdal35gLk"> &nbsp; &nbsp; <img src="http://www.clipular.com/c/5482778819821568.png?k=0ABAsScKdMlMhm02iRyZaI6HTFo">
    + `Avian` -- 12 matches
    + `avian` -- with the ignore case flag -- 14 matches <br><img src="http://www.clipular.com/c/4523633702600704.png?k=2ANfuhKq9-YlXhHskHO6UWvkeZ0">  &nbsp; &nbsp;<img src="http://www.clipular.com/c/6101324037881856.png?k=6kHKagztUKV9hfLP2f0z64Euk7E">  &nbsp; &nbsp; <img src="http://www.clipular.com/c/5227111764721664.png?k=J7C2jn8BBj9xTRXVugf8d9K2ui0">
    + Clear the case insensitive flag   

### Capitalization
3. **Find only capitalized** words   
		*Some characters don't have a literal meaning, these are known as meta characters:  e.g. `"` or `[`*
	  + `[A-Z]\w*` -- character classes `[` `]` are denoted by square brackets; wildcards include `*` , `+` , `?` <br><img src="http://www.clipular.com/c/4898985692102656.png?k=IPiLHHWl3MWUjXTW0gbaLf5j_pY">
	  + `[A-Z]+` -- match only "all caps" words.  **BUT** this is not quite right.  It doesn't work.  *Do you know why?*
	  + `\b[A-Z]+\b` -- Because you have to match on a word boundary using an anchor class:  `\b`
	  + `\b[A-Z]{2,}\b` -- Abbreviations are usually 2 or more upper case characters.  Squiggly brackets `{` `}` allow for arbitrary repetition   

### Last word
4. **Match the last words of sentences**<br>*We can restore a letter's literal meaning by escaping it*
    + `\w+.` -- This doesn't work because "." matches every character
    + `\w+\.` -- We escape the period "." with a the escape character `\`
    + `\w+\.\s` -- More precise this time.  Matching on 56 words.  Using `\s` allows us to stop matching email address by matching whitespace `\s`  

### Numbers
5. **Find all years** 
*Note: the pipe character `|` *
*Note: alternation, alternatives *
*Note: `(` `)` grouping *
*Note `{` `}` multiplier
    + `\d\d\d\d` -- a lot of matches here
    + `\d{4}` -- more susccinct but has the same meaning as above
    + `\b\d{4}\b` -- word boundaries `\b` help but there are still some false positives
    + `\b(19|20)\d\d\b` -- better and works for the twenties and twenty-first centuries   

### Phone numbers
6. Find a phone number<br>
*Note: escape the parenthesis `\(`*   
*Note: `?` indicates optionality matching zero or one occurrence*
    + `\(\d{3}\) \d{3}-\d{4}` -- Very specific.  This works as long as phone numbers are formatted consistently
    + `\(?\d+\)? ?[\d-]{5,}\d` -- more permissive
    + `\(?\d+(\)|.)? ?[\d-.]{5,}\d`	   -- more permissive still.  Allows for `.` instead of - as a separator

### Email addresses
7. `\w+@[\w\.]+` -- This rule is quite permissive.  It's likely to match some invalid email addresses. e.g. fred@invalid.net   
			It's also likely to miss valid email addresses like vincent.zappa@intersect.org.au   
			**SUGGESTION**: ask the RegExr community:
    + left-hand sidebar
    + click Community
    + search on the term 'email'
    + From there, I found this one: <br>   
				`([\w\.]+)@([\w\.]+)\.(\w+)` <br>   
...
### Section headings
8. Note how `+` can be applied to a group  
    + Using the Flag setting of RegExr (upper-left), set to multiline	-- This enables ^ and $ anchors
    + `^(\w+ ?)+$` -- match repeating words + optional space
    + Reset the multiline flag   
...
Below the line -- UNDER CONSTRUCTION/RENOVATION.  Check previous [iteration](http://v.gd/intro_regex) if you need to see what was done in Junuary 2016
