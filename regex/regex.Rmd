...
3. **Find only capitalized** words   
	  ...
	  a. `[A-Z]\w*` -- character classes `[` `]` are denoted by square brackets; wildcards include `*` , `+` , `?` <br><img src="http://www.clipular.com/c/4898985692102656.png?k=IPiLHHWl3MWUjXTW0gbaLf5j_pY">
	  b. `[A-Z]+` -- match only "all caps" words.  **BUT** this is not quite right.  It doesn't work.  *Do you know why?*
	  c. `\b[A-Z]+\b` -- Because you have to match on a word boundary using an anchor class:  `\b`
	  d. `\b[A-Z]{2,}\b` -- Abbreviations are usually 2 or more upper case characters.  Squiggly brackets `{` `}` allow for arbitrary repetition   
